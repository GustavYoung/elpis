...
omx_dbus.on('changeStatus', function(status) {
    ...
    if (diff > 2000000 && diff < 17000000 && aboutToFinish === false){
        aboutToFinish = true;
        //eventEmitter.emit('aboutToFinish');
        eventEmitter.emit('aboutToFinish15s');
    }
    if (diff > 2000000 && diff < 12000000 && aboutToFinish === false){
	aboutToFinish = true;
        //aboutToFinishSentId1.s10 = true;
        eventEmitter.emit('aboutToFinish10s');
    }
    if (diff > 2000000 && diff < 7000000 && aboutToFinish === false){
	aboutToFinish = true;
        //aboutToFinishSentId1.s5 = true;
        eventEmitter.emit('aboutToFinish5s');
    }
});
omx_dbus.on('finish', function() {
    aboutToFinishSentId1 = { s15: false, s10: false, s5: false };
