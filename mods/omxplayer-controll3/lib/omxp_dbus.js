    ...
    if (typeof omx_methods.properties[prop] !== 'undefined' && omx_methods.properties[prop].read) {
        if (typeof bus !== 'undefined') {
            bus.invoke({
                ...
                member: prop,
                destination: 'org.mpris.MediaPlayer2Id3.omxplayer'
            }, function(err, resp) {
                return cb(err, resp);
            });
        } else {
...
function setVolume(val, cb) {
    var cmd = 'OMXPLAYER_DBUS_ADDR="/tmp/omxplayerdbus.${USER:-root}";OMXPLAYER_DBUS_PID="/tmp/omxplayerdbus.${USER:-root}.pid";export DBUS_SESSION_BUS_ADDRESS=`cat $OMXPLAYER_DBUS_ADDR`;export DBUS_SESSION_BUS_PID=`cat $OMXPLAYER_DBUS_PID`;dbus-send --print-reply=literal --session --reply-timeout=500 --dest=org.mpris.MediaPlayer2Id3.omxplayer /org/mpris/MediaPlayer2 org.freedesktop.DBus.Properties.Volume double:';
